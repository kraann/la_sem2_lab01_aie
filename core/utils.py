...
def validate_shape(
    ...
    if not isinstance(shape, (tuple, list)):
        raise TypeError("Форма тензора должна быть списком или кортежем")

    result = []
    for item in shape:
        if not isinstance(item, int):
            raise ValueError("Все элементы формы должны быть целыми числами")
        if item <= 0:
